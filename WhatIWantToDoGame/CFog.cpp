...
CFog::CFog(ILevel& partner, XMFLOAT3 loc) :CActor(partner)
{
	...
	if (random == 0)
	{
		CBillboardComponent& billboard = *new CBillboardComponent(*this, Transform, "Assets/Textures/Trail/霧差分1.png");
		billboard.Transform.Scale = { 100.0f,100.0f,1.0f };
		billboard.SetColor({ 1,1,1,0.02 });
	}
	else
	{
		CBillboardComponent& billboard = *new CBillboardComponent(*this, Transform, "Assets/Textures/Trail/霧差分2.png");
		billboard.Transform.Scale = { 100.0f,100.0f,1.0f };
		billboard.SetColor({ 1,1,1,0.02 });
	}
}
