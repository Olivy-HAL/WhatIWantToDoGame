...
CTraiActor::CTraiActor(ILevel& owner) :CActor(owner), mPointer(*new CPointer(owner, *this)),
mSpeedLimitMin(mSpeed / 2.0f), mSpeedLimitMax(mSpeed*2.0f)
{
	...
	*/
	...
	/*new CParticleGeneratorComponent(*this, Transform, std::bind(&CFighter::Particle, std::ref(*this), std::placeholders::_1), 0.3f, 1.f, 30,
		...
		*/
	CInputManager::GetInstance().AddEvent("Shot", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_SPACE }, std::bind(&CTraiActor::Shot, std::ref(*this)));
	CInputManager::GetInstance().AddEvent("Rot-Y", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_A }, std::bind(&CTraiActor::Rot, std::ref(*this), 0));
	CInputManager::GetInstance().AddEvent("Rot+Y", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_D }, std::bind(&CTraiActor::Rot, std::ref(*this), 1));
	CInputManager::GetInstance().AddEvent("Rot-X", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_W }, std::bind(&CTraiActor::Rot, std::ref(*this), 2));
	CInputManager::GetInstance().AddEvent("Rot+X", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_S }, std::bind(&CTraiActor::Rot, std::ref(*this), 3));
	CInputManager::GetInstance().AddEvent("SpeedUP", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_UP }, std::bind(&CTraiActor::SpeedChange, std::ref(*this), 0));
	CInputManager::GetInstance().AddEvent("SpeedDPWN", EButtonOption::PRESS, *this, { EButtonType::KEYBOARD,DIK_DOWN }, std::bind(&CTraiActor::SpeedChange, std::ref(*this), 1));
...
void CTraiActor::ShotReset()
{
	mShotCnt = 0;
}

void CTraiActor::Move()
{
	XMFLOAT3 fv = Transform.GetForwardVectorRelative();
...
void CTraiActor::Particle(CActor& actor)
{
	if (!mIsParticleFlag)
	{
...
void CTraiActor::SpeedChange(int type)
{
	if (type == 0)
	{
...
void CTraiActor::EventAtBeginCollide(CActor& collideActor)
{
